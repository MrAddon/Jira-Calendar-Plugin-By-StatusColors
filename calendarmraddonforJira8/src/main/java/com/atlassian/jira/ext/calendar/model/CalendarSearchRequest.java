...
public class CalendarSearchRequest
{
            ...
            throws InvalidFilterSearchRequestException, InvalidProjectSearchRequestException, SearchException
    {
        ...
        this.query = projectOrFilterIdSearchRequest.getQuery();
        this.projectId = projectId;
        this.customFieldManager = customFieldManager;

        query = addConstraintToSearchRequest(query, dateFieldName, this.startDate, this.endDate);
        query = orderByDateFieldAndPriority(query, dateFieldName);
    ...
    {
        SearchResults<Issue> results = getResults();
    ...
    private Date truncate(Date date) {
        ...
        calendar.setTimeZone(ComponentAccessor.getComponent(TimeZoneManager.class).getLoggedInUserTimeZone());
     ...
     *
     * @param searchQuery   The query to constraint
     ...
     */
    private Query addConstraintToSearchRequest(Query searchQuery, String dateFieldName, Date start, Date end)
    {
        Clause whereClause;
        ...
        if (null != whereClause)
        {
            if (null != searchQuery.getWhereClause())
                return addWhereClause(searchQuery, whereClause);
            else
    ...
    private static Query addWhereClause(Query query, Clause whereClause) {
        return JqlQueryBuilder.newBuilder(query)
                .where()
                .and().addClause(whereClause)
        .buildQuery();
    }
     ...
     *
     * @param query            The search request to that will be used to query for the ordered result.
     * @param dateFieldName The date field name &mdash; one of {@link com.atlassian.jira.issue.index.DocumentConstants#ISSUE_DUEDATE},
     *                      {@link com.atlassian.jira.issue.index.DocumentConstants#ISSUE_CREATED}, {@link com.atlassian.jira.issue.index.DocumentConstants#ISSUE_UPDATED},
     */
    private Query orderByDateFieldAndPriority(Query query, String dateFieldName)
    {
    ...
    public SearchResults<Issue> getResults() throws SearchException
    {
     ...
     */
    public Query getSearchRequestForDay(Date date)
    {
        // truncate to day
        ...
        Query searchRequestForDay = projectOrFilterIdSearchRequest.getQuery();
        searchRequestForDay = addConstraintToSearchRequest(searchRequestForDay, dateFieldName, dayStartDate, dayEndDate);
    ...
    public String getSearchName() {
        return projectOrFilterIdSearchRequest.getName();
    }
}
